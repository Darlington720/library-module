...
export function ClearanceTable({ searchQuery, onStudentSelect }: ClearanceTableProps) {
  ...
  const getStatusBadge = (status: ClearanceRequest['status']) => {
    switch (status) {
      ...
      case 'disqualified':
        return <Badge className="bg-orange-100 text-orange-800">Disqualified</Badge>;
      default:
        ...
        <TableBody>
          {filteredData.map((item) => (
            <TableRow key={item.id}>
              ...
              <TableCell>
                {item.clearance.submittedAt.toLocaleDateString()}
              </TableCell>
              <TableCell>
              ...
              <TableCell>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => onStudentSelect(item, item.clearance)}
                >
                  Review
                </Button>
              </TableCell>
